# -*- coding: utf-8 -*-
# baselines/run_rq3_rq4.py  — V3 (mask fix + architectural consistency)
"""
RQ3 & RQ4 Experiment Runner — V3

═══════════════════════════════════════════════════════════════════
THAY ĐỔI SO VỚI V2:

  V2 BUG 1 — FallbackVariantModel đảo ngược news_mask:
    nm = ~news_mask.bool()  →  True = có tin tức = key_padding_mask bỏ qua
    Cross-attention attend vào zero-embedding days, ignore news days.

  V2 BUG 2 — FallbackVariantModel không truyền news_mask/news_quality vào encoder:
    NewsEncoder bỏ qua Step4 masking và gate hard-zero →
    LayerNorm bias tạo pseudo-signal từ zero embeddings.

  V2 BUG 3 — GatedCrossAttention ≠ StableGatedCrossAttention:
    W_a = Linear(bias=False) + bias_a (single bias) vs
    W_a = Linear(bias=True) + bias_a (double bias) trong src/fusion.py →
    RQ3 "gated" không khớp chính xác với production model.

  V2 BUG 4 — RQ4 so sánh không công bằng:
    Order_NM dùng PatchedStockMovementModel (mask đúng),
    Order_MN và Order_par dùng FallbackVariantModel (mask sai) →
    kết quả MN > NM là artefact của bug, không phải fusion order.

  V3 FIX:
    1. Import StableGatedCrossAttention từ src.fusion — "gated" variant
       dùng ĐÚNG class của production model, không định nghĩa lại.
    2. Sửa FallbackVariantModel: nm = news_mask.bool() (bỏ dấu ~).
    3. Sửa FallbackVariantModel: truyền news_mask và news_quality vào encoder.
    4. RQ3: PatchedStockMovementModel cho cả gated và nogated (NM fixed).
       "gated" patch với StableGatedCrossAttention → parity 100% với production.
    5. RQ4: FallbackVariantModel cho CẢ BA orders (NM, MN, par) →
       so sánh fusion order công bằng trên cùng kiến trúc.

═══════════════════════════════════════════════════════════════════

RQ3: Gated CA có lower variance (ổn định hơn) so với Plain CA không?
     - MSGCA_Gated  : StableGatedCrossAttention (= production model)
     - MSGCA_NoGated: PlainCrossAttention (bỏ sigmoid gate)
     Fixed: fusion_order = NM, dùng PatchedStockMovementModel

RQ4: Fusion order NM có tối ưu hơn các order khác không?
     - Order_NM : price×news → (price+news)×macro  [canonical]
     - Order_MN : price×macro → (price+macro)×news
     - Order_par: parallel price×news + price×macro → mean-merge
     Fixed: fusion_type = gated, tất cả dùng FallbackVariantModel
...
import argparse, copy, json, os, sys, time, random
import numpy as np
import torch
...
# [V3 FIX-3] Import trực tiếp từ src.fusion — đảm bảo "gated" = production model
...
_PATIENCE    = TrainConfig.early_stop_patience
_MOD_DROPOUT = TrainConfig.news_modality_dropout
...
# [V3] GatedCrossAttention ĐÃ BỊ XÓA — dùng StableGatedCrossAttention từ src.fusion.
# Đây là class ĐÚNG duy nhất cho "gated" variant để khớp với production model.
...
class PlainCrossAttention(nn.Module):
    ...
    def __init__(self, dim: int, num_head: int, dropout: float = 0.1):
        ...
        # Dùng cùng cấu trúc W_a với StableGatedCrossAttention để so sánh sạch
        self.W_a     = nn.Linear(dim, dim)          # bias=True, khớp src/fusion.py
    ...
    def forward(
        ...
        aux:      torch.Tensor,
        aux_mask: Optional[torch.Tensor] = None,  # (B, T) bool: True=no news=ignore
    ) -> torch.Tensor:
...
# =============================================================================
# PATCHED MODEL — swap only fusion layers, keep everything else (RQ3)
# =============================================================================

def _discover_fusion_attrs(model: StockMovementModel) -> Tuple[Optional[str], Optional[str]]:
    """Introspect StockMovementModel để tìm tên của hai fusion stages."""
    candidates_stage1, candidates_stage2 = [], []
    ...
    if not candidates_stage1 or not candidates_stage2:
        ...
        for name, module in model.named_children():
            ...
            elif any(isinstance(m, nn.MultiheadAttention) for m in module.children()):
                mha_attrs.append(name)
        if len(mha_attrs) >= 2:
...
class PatchedStockMovementModel:
    ...
    Dùng cho RQ3 (NM fixed): đảm bảo parity tuyệt đối với production model.

    Khi patch với StableGatedCrossAttention (freshly initialized):
      → kiến trúc GIỐNG HỆT StockMovementModel, chỉ khác weight initialization
      → cả hai đều được train from scratch → fair comparison
    """
    def __init__(self, base_model, stage1_attr, stage2_attr, new_stage1, new_stage2):
...
def _build_base_model(macro_dim: int, news_dim: int, cw: torch.Tensor) -> StockMovementModel:
    """Build StockMovementModel — identical to run_ablation.py build_model()."""
    return StockMovementModel(
        ...
        class_weights=cw, use_focal_loss=True, focal_gamma=2.0,
...
# =============================================================================
# FALLBACK VARIANT MODEL — dùng cho RQ4 (tất cả fusion orders)
# =============================================================================
...
class FallbackVariantModel(nn.Module):
    """
    Unified variant model cho RQ4 — tất cả fusion orders dùng cùng class.

    [V3 FIXES]:
      1. news_mask: nm = news_mask.bool() (True=no news=ignore), KHÔNG đảo ngược.
      2. encoder: truyền news_mask + news_quality để NewsEncoder masking hoạt động.
      3. FusionCls gated: dùng StableGatedCrossAttention (= production model).
    """
    def __init__(
        self,
        macro_dim:      int,
        ...
        # [V3 FIX-3] Dùng StableGatedCrossAttention cho "gated" — khớp production
        if fusion_type == "gated":
            FusionCls = StableGatedCrossAttention
        else:
            FusionCls = PlainCrossAttention
    ...
    def forward(
        ...
        **kwargs,
        ...
        # [V3 FIX-1] news_mask convention: True = no news = key_padding_mask ignore
        # data_loader: mask[t,w]=True khi không có tin → dùng trực tiếp, KHÔNG đảo ngược
        nm = news_mask.bool().to(DEVICE) if news_mask is not None else None

        # [V3 FIX-2] Truyền news_mask và news_quality vào encoder
        # NewsEncoder cần mask để Step4 (masked_fill) và gate hard-zero hoạt động
        enc_out = self.encoder(
            s_o.to(DEVICE), s_h.to(DEVICE), s_c.to(DEVICE),
            s_m.to(DEVICE), s_n.to(DEVICE),
            news_mask=nm,
            news_quality=news_quality.to(DEVICE) if news_quality is not None else None,
        )
        ...
        v_t     = self.ticker_proj(self.ticker_emb(tid))
        v_t_seq = v_t.unsqueeze(1).expand(-1, T, -1)

        # ── Fusion ────────────────────────────────────────────────────────────
        if self.fusion_order == "NM":
            # Stage1: price × news (với mask đúng)
            H_id  = self.stage1(primary=v_i,  aux=v_n, aux_mask=nm)
            # Stage2: (price+news) × macro (macro luôn available, không cần mask)
            H_idm = self.stage2(primary=H_id, aux=v_m)
        elif self.fusion_order == "MN":
            # Stage1: price × macro (không cần mask)
            H_im  = self.stage1(primary=v_i,  aux=v_m)
            # Stage2: (price+macro) × news (với mask đúng)
            H_idm = self.stage2(primary=H_im, aux=v_n, aux_mask=nm)
        else:  # par — parallel mean-merge
            H_n   = self.fusion_news( primary=v_i, aux=v_n, aux_mask=nm)
            H_m   = self.fusion_macro(primary=v_i, aux=v_m)
            H_idm = (H_n + H_m) * 0.5

        fused  = self.fused_proj(torch.cat([H_idm, v_t_seq], dim=-1))
...
def load_splits(pkl_path: str, tickers: list) -> dict:
    """
    Split logic IDENTICAL với run_ablation.py.
    test = [inner_T : T_max] = cùng test set với main.py và run_ablation.py.
    """
...
def _make_adamw(model, lr: float) -> torch.optim.Optimizer:
    ...
    if not dec and not nodec:
        return torch.optim.AdamW(
            model.parameters(), lr=lr, weight_decay=TrainConfig.weight_decay
        )
    return torch.optim.AdamW(
...
def evaluate_model(model, data: dict) -> Tuple[float, float]:
    """Evaluate → (acc, mcc). Works with both PatchedStockMovementModel and FallbackVariantModel."""
...
def run_one_seed(
    ...
    """
    Train một variant/seed.

    force_fallback=False (RQ3): dùng PatchedStockMovementModel nếu introspect OK.
      → gated variant patch với StableGatedCrossAttention = production model exactly.
      → nogated variant patch với PlainCrossAttention.

    force_fallback=True (RQ4): luôn dùng FallbackVariantModel.
      → tất cả orders (NM, MN, par) dùng cùng class → fair comparison.
    """
    if patience is None:
        patience = _PATIENCE
    set_seed(seed)
    ...
    use_patch = (
        not force_fallback
        and stage1_attr is not None
        and stage2_attr is not None
        and fusion_order == "NM"    # PatchedModel chỉ hợp lệ cho NM (forward cố định)
    )

    if use_patch:
        # RQ3: Patch đúng class vào StockMovementModel
        base = _build_base_model(macro_dim, news_dim, cw)
        if fusion_type == "gated":
            # [V3] Dùng StableGatedCrossAttention — ĐÚNG class của production model
            new_s1 = StableGatedCrossAttention(dim=dim, num_head=nh, dropout=0.1).to(DEVICE)
            new_s2 = StableGatedCrossAttention(dim=dim, num_head=nh, dropout=0.1).to(DEVICE)
        else:
            new_s1 = PlainCrossAttention(dim=dim, num_head=nh, dropout=0.1).to(DEVICE)
            new_s2 = PlainCrossAttention(dim=dim, num_head=nh, dropout=0.1).to(DEVICE)
        ...
        # RQ4 (force_fallback=True) hoặc RQ3 fallback
    ...
    for epoch in range(max_epochs):
        ...
        for batch in ldr:
            ...
            # News modality dropout — khớp main.py / run_ablation.py
...
def load_reference_results(results_dir: str) -> dict:
    ...
    if not refs:
        print("  [Ref] No reference results found. "
              "Run run_experiments.py and run_ablation.py first.")
    return refs


def validate_gated_nm(gated_nm_result: dict, refs: dict) -> str:
    """
    Kiểm tra gated/NM có overlap phân phối với MSGCA_FV không.
    V3: dùng StableGatedCrossAttention nên gap phải nhỏ hơn V2.
    """
    ...
    if gap <= overlap_threshold:
        verdict = (f"✓ PASS — distributions overlap "
                   ...
                   f"Architectural parity confirmed (StableGatedCrossAttention used).")
    else:
        verdict = (f"✗ WARN — gap={gap:.4f} > {overlap_threshold:.4f}. "
                   f"Expected gap due to different loss fn: "
                   f"RQ3 uses FocalLoss+CW, MSGCA_FV uses CE. "
                   f"Architecture is correct (StableGatedCrossAttention).")

    return (f"  [Validation] MSGCA_Gated(NM) vs MSGCA_FV:\n"
            f"    Gated: {gm:.4f}+/-{gs:.4f}  |  FV(CE): {rm:.4f}+/-{rs:.4f}\n"
            f"    {verdict}")
...
def format_rq3_table(results: dict, refs: dict) -> str:
    ...
    ref_key = next((k for k in variants_rq3 if "nogated" not in k.lower()), None)
    ...
    lines = [
        ...
        "  Fixed var  : fusion_order = NM",
        "  Model      : PatchedStockMovementModel (V3: gated = StableGatedCrossAttention)",
        "  Loss       : FocalLoss(γ=2.0) + class weights",
        "  Protocol   : identical to run_ablation.py (fixed-val, warmup=10, patience=TrainConfig)",
        "  Test set   : [inner_T : T_max] — same as main.py and run_ablation.py",
    ]
    if anchor_mcc is not None:
        lines.append(
            f"  Anchor     : MSGCA_FV (CE loss, from run_experiments.py) = "
    ...
    lines += [sep, "\nINTERPRETATION (RQ3):"]
    gated_keys   = [k for k in variants_rq3 if "nogated" not in k.lower()]
    nogated_keys = [k for k in variants_rq3 if "nogated" in k.lower()]
    if gated_keys and nogated_keys:
        ...
        if g_std < ng_std:
            ...
            lines.append(f"  ANSWER: NO — Plain CA has lower variance: "
                         f"std_gated={g_std:.4f} ≥ std_plain={ng_std:.4f} (Δ={std_diff:.4f})")
        mcc_d = g_mcc - ng_mcc
        lines.append(f"  MCC: gated={g_mcc:.4f} vs plain={ng_mcc:.4f} (Δ={mcc_d:+.4f})")
        if abs(mcc_d) < 0.010:
            lines.append("  |ΔMCC| < 0.010 — within noise. "
                         "Stability (std) is the primary RQ3 metric.")
...
def format_rq4_table(results: dict, refs: dict) -> str:
    ...
    lines = [
        ...
        "  Fixed var  : fusion_type = gated (StableGatedCrossAttention)",
        "  Model      : FallbackVariantModel (V3: tất cả orders dùng cùng class)",
        "  Loss       : FocalLoss(γ=2.0) + class weights",
        "  Protocol   : identical to run_ablation.py",
    ]
    if anchor_mcc is not None:
        lines.append(f"  Anchor     : MSGCA_FV (CE, StockMovementModel) = {anchor_mcc:.4f}")
    lines += [
        sep,
    ...
    if variants_rq4:
        ...
        if not is_nm and ref_mcc is not None:
            gap = abs(best["mcc_mean"] - ref_mcc)
            nm_std = results[ref_key]["mcc_std"] if ref_key else 0.0
            if gap < best["mcc_std"] + nm_std:
                lines.append(
                    f"  Note: |gap| = {gap:.4f} is within combined std — "
                    "not conclusive, larger dataset or more seeds needed."
                )
...
def main():
    ap = argparse.ArgumentParser(
        description="RQ3 (Gated vs Plain CA) + RQ4 (Fusion order) — V3"
    )
    ...
    print("=" * 70)
    print("  RQ3 & RQ4 Experiment Runner — V3 (mask fix + arch consistency)")
    ...
    print(f"  RQ3 model: PatchedStockMovementModel (gated=StableGatedCrossAttention)")
    print(f"  RQ4 model: FallbackVariantModel (all orders unified)")
    print("=" * 70)
    ...
    # Introspect fusion attr names từ StockMovementModel (dùng cho RQ3 patching)
    ...
    s1a, s2a = _discover_fusion_attrs(_tmp)
    del _tmp

    if s1a and s2a:
        print(f"  → Discovered: stage1='{s1a}'  stage2='{s2a}'")
        print(f"  → RQ3 will use PatchedStockMovementModel with these attrs")
    else:
        print("  → Could not discover fusion attrs — RQ3 will use FallbackVariantModel")

    all_results: dict = {}
    total_t0 = time.time()

    # ── RQ3: Gated vs NoGated (NM fixed, PatchedStockMovementModel) ───────────
    if run_rq3:
        print(f"\n{'=' * 70}")
        print("  RQ3: Gated vs Non-Gated Cross-Attention  (fusion_order=NM fixed)")
        print(f"  Model: PatchedStockMovementModel — gated patches StableGatedCrossAttention")
        print(f"{'=' * 70}")

        rq3_variants = [
            ("MSGCA_Gated",   "gated", "NM"),
            ("MSGCA_NoGated", "plain", "NM"),
        ]
        for label, ftype, forder in rq3_variants:
            ...
            result = run_variant_n_seeds(
                ...
                stage1_attr=s1a, stage2_attr=s2a,
                force_fallback=False,   # RQ3: dùng PatchedStockMovementModel
    ...
    # ── RQ4: Fusion Order (gated fixed, FallbackVariantModel cho tất cả) ──────
    if run_rq4:
        print(f"\n{'=' * 70}")
        print("  RQ4: Fusion Order (fusion_type=gated fixed)")
        print(f"  Model: FallbackVariantModel (NM, MN, par dùng cùng class — fair)")
        print(f"{'=' * 70}")
        ...
        for label, ftype, forder in rq4_variants:
            ...
            result = run_variant_n_seeds(
                ...
                splits=splits, n_seeds=args.n_seeds, max_epochs=args.epochs,
                stage1_attr=s1a, stage2_attr=s2a,
                force_fallback=True,    # RQ4: tất cả orders dùng FallbackVariantModel
                verbose=args.verbose,
            )
            all_results[key] = result
            print(f"  → ACC={result['acc_mean']:.4f}+/-{result['acc_std']:.4f}  "
