...
from src.data_loader_v6 import data_prepare
...
def train_model(train_data: dict, valid_data: dict, test_data: dict):
    ...
    if os.path.exists(save_path):
        ...
        # --- THAY ĐỔI: TRẢ VỀ GIÁ TRỊ ĐỂ TUNING LƯU LẠI ---

        return val_acc, val_mcc, test_acc, test_mcc
    ...
    if final_train:
        ...
        for k in ["s_o", "s_h", "s_c", "s_m", "s_n"]:
            ...
            if torch.isnan(final_train[k]).any():
                ...
                print(f"[CẢNH BÁO] Phát hiện NaN trong tensor {k}.")
