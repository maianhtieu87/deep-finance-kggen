...
def compute_class_weights(labels_tensor: torch.Tensor) -> torch.Tensor:
    """
    [TIER 1] Balanced Class Weights (Effective-Sqrt Strategy)
    """
    ...
    weights = weights / np.sum(weights) * num_classes # Normalize 1

    # 2. Sqrt Smoothing
    weights = np.sqrt(weights)
    weights = weights / np.sum(weights) * num_classes # Normalize 2
...
def train_model(train_data: dict, valid_data: dict, test_data: dict):
    ...
    model = StockMovementModel(
        ...
        dropout=0.1,                          
        class_weights=class_weights, # Truyền weights vào đây
        use_focal_loss=True,         # Kích hoạt Focal Loss
        device=device
    ).to(device)

    lr = getattr(TrainConfig, "learning_rate", 1e-4)
    wd = getattr(TrainConfig, "weight_decay", 1e-4)
    ...
    for epoch in range(int(TrainConfig.epoch_num)):
        ...
        # [STRATEGY UPDATE] Save Best Logic (Prioritize MCC)
        is_best = False
